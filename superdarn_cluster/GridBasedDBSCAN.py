...
class GridBasedDBSCAN():
    ...
    def _region_query(self, m, grid_id):
        ...
        t_min, t_max = max(0, grid_id[2] - self.time_eps), min(self.time_eps)
    ...
    def _expand_cluster(self, m, classifications, grid_id, cluster_id, min_points):
        ...
        if len(seeds) < min_points:
            ...
            while len(seeds) > 0:
                ...
                eps = self.gate_eps, self.beam_eps / self.C[current_point[0], current_point[1]] # TODO debug var, remove
...
if __name__ == '__main__':
    ...
    gates = []
    data = []
    data_indices = []       # Indices where data exists in the sparse matrix
    pts_per_time = 100
    np.random.seed(2)
    for t in time:
        ...
        beams.append(beam)
        gates.append(gate)
        grid = sparse.csr_matrix((np.array([True]*pts_per_time), (gate, beam)), shape=(ngate, nbeam))
        data.append(grid)
        data_indices.append(list(zip(gate, beam)))
        pass
    #data = np.array(data)
    data = data[0]  # just use 1 grid for now
    data_indices = data_indices[0]

    gate_eps = 5
    beam_eps = 5
    time_eps = 3
    # TODO adapt min_pts to be a ratio of possible observations
